...
BEGIN
    ...
    UPDATE dbo.SiteGeofences
       SET Name = COALESCE(NULLIF(LTRIM(RTRIM(Name)), ''), CONCAT('Legacy geofence ', LEFT(CONVERT(varchar(36), Id), 8))),
           NormalizedName = COALESCE(NULLIF(LTRIM(RTRIM(NormalizedName)), ''), UPPER(REPLACE(REPLACE(LTRIM(RTRIM(COALESCE(Name,''))), ' ', ''), '-', ''))),
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.SiteGeofences') AND name = N'IX_SiteGeofences_NormalizedName')
        CREATE UNIQUE INDEX IX_SiteGeofences_NormalizedName ON dbo.SiteGeofences(NormalizedName);
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.SiteGeofences') AND name = N'IX_SiteGeofences_SiteId')
...
BEGIN
    ...
    UPDATE dbo.GeofenceVisits
       SET VehicleIdentifier = COALESCE(NULLIF(LTRIM(RTRIM(VehicleIdentifier)), ''), 'UNKNOWN'),
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.GeofenceVisits') AND name = N'IX_GeofenceVisits_Vehicle_Open')
        CREATE INDEX IX_GeofenceVisits_Vehicle_Open ON dbo.GeofenceVisits(VehicleIdentifier, ExitedAtUtc);
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.GeofenceVisits') AND name = N'IX_GeofenceVisits_Load_Stop')
        CREATE INDEX IX_GeofenceVisits_Load_Stop ON dbo.GeofenceVisits(LoadId, LoadStopId);
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.GeofenceVisits') AND name = N'IX_GeofenceVisits_Entered')
